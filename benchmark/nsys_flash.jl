...
# direction per run so there's a single capture region and --capture-range grabs it
# cleanly (the reference-ms gputime loop runs OUTSIDE the region, so nsys ignores it):
#   FLASH_DIR=bwd nsys profile --capture-range=cudaProfilerApi --trace=cuda --sample=none \
#        -o flash_c32_w35_bwd julia --project benchmark/nsys_flash.jl
#   nsys stats --report gpukernsum flash_c32_w35_bwd.nsys-rep | head -40
# For DIR=both or MODE=all (multiple capture regions) add: --capture-range-end=repeat

using CUDA, NNlib, CirculantAttention, Zygote
...
println("""
done. For the accurate device-kernel breakdown, profile ONE direction per run:
  FLASH_DIR=bwd nsys profile --capture-range=cudaProfilerApi --trace=cuda --sample=none \\
       -o flash_bwd julia --project benchmark/nsys_flash.jl
  nsys stats --report gpukernsum flash_bwd.nsys-rep | head -40
  (DIR=both / MODE=all ⇒ add --capture-range-end=repeat for the extra regions)
